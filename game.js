...
function pauseHomeBGM() {
    homeBgm.pause();
    try { homeBgm.currentTime = 0; } catch (e) {}
}
...
function pauseBattleBGM() {
    battleBgm.pause();
    try { battleBgm.currentTime = 0; } catch (e) {}
}

function resumeBattleBGM() {
    // resume 之前确保其他 BGM 已停止
    stopAllBGM();
    battleBgm.play();
}
...
// 停止并重置所有背景音乐（不启动任何）
function stopAllBGM() {
    try { if (homeBgm && !homeBgm.paused) homeBgm.pause(); } catch (e) {}
    try { if (battleBgm && !battleBgm.paused) battleBgm.pause(); } catch (e) {}
    try { if (storyBgm && !storyBgm.paused) storyBgm.pause(); } catch (e) {}
    try { if (homeBgm) homeBgm.currentTime = 0; } catch (e) {}
